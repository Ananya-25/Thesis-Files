...
class ManifoldGP(object):
    ...
    def learn_landmarks(self, X,  batch_size, kern_width=None):
        ...
        if kern_width is None:
            #self.kern_width = 0.599 * (np.var(X, axis=0).sum())
            #print("kernel width: ", self.kern_width)
            #n_trials = 10000
            n_trials = 50
            total_ep = 0
            ...
            for i in range(n_trials):
                ...
                pcm = PCManifold(X[idx,:])
                pcm.optimize_parameters()
                ep = pcm.kernel.epsilon
                total_ep += ep
        
            #self.kern_width = total_ep / n_trials
            self.kern_width = total_ep
        else:
            self.kern_width = kern_width
        print("bandwidth: ", self.kern_width)
